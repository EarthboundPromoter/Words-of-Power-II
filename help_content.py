...
[Shift + Arrow:shields]: Move the cursor 4 tiles; a two-arrow pair does it diagonally. Speaks the landing tile by name, then a short summary of everything crossed, floor included. Press D on arrival for full detail. Says Edge when the map edge cuts the move short
[Ctrl + direction:shields]: Hold Ctrl and press a direction to peek: a look cursor appears at your feet and steps one tile. Keep holding Ctrl to keep looking; every arrow steps the cursor. Release Ctrl and you're back in control, cursor gone. Press Enter or V mid peek to stay in look mode after release, or a spell key to aim that spell right where you were looking. In look or aiming already, Ctrl+arrow is just a single step. Either Ctrl
[Ctrl + Shift + direction:shields]: The jump: skim the cursor along that line until what you would hear changes, past open floor to the next unit, item, or wall. Speaks the crossing then the landing, like 4 floor east, Imp. The number is the distance to the landing tile; beings and items crossed are counted truthfully, like past 3 goblins. Says Edge when the map ends the run, every time you try it. From normal play it conjures the peek cursor and jumps in one gesture. Either Ctrl; arrows and arrow pairs. On the numpad use the arrow row instead: Windows eats that numpad chord
